...
2. **Session Naming Hook** (`session-naming.sh`)
   ...
   - Generates concise session names via Claude Haiku
   - Cache: Once per session, permanent until cleanup
...
# If we already have a cached name, keep it (idle sessions should keep their name)
...
**Session Name:**
- Generate once on first substantial user input
- Keep forever (idle sessions maintain their name)
